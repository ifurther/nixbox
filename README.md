...
```shell
vagrant init nixbox/nixos --box-version 22.11
```
...
```shell
packer build --only=virtualbox-iso.virtualbox -var version=22.11 --except=vagrant-cloud nixos.pkr.hcl
packer build --only=qemu.qemu -var version=22.11 --except=vagrant-cloud nixos.pkr.hcl
packer build --only=vmware-iso.vmware -var version=22.11 --except=vagrant-cloud nixos.pkr.hcl
packer build --only=hyperv-iso.hyperv -var version=22.11 --except=vagrant-cloud nixos.pkr.hcl
```

The vagrant .box image is now ready to go and you can use it in vagrant:

```shell
vagrant box add nixbox32 nixos-22.11-libvirt-i686.box
# or
vagrant box add nixbox64 nixos-22.11-virtualbox-x86_64.box
```
